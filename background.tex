...
In this chapter we introduce the background knowledge and terminology that is
required for the work presented later.  We then review the current state of
two broad categories of data analysis techniques, namely partitional
...
\citep{bhattacharyya43distance} and in a general positive edge-weighted graph,
...
distributions, for which metrics include the Hellinger distance and
Jensen-Shannon divergence squared \citep{endres03metric}.  In
Section~\ref{sec:comparing-partitions} we look at metrics (and other measures)
for comparing partitions and in Chapter~\ref{cha:sum-squar-clust} we
introduce our own metric.  It is also possible to transform some
dissimilarities into metrics, for example see \citet[chap. 2.5]{everitt80}.
...
It should be noted that all three are special cases of the \textit{Minkowski
  distance}, $\dissimdefn{I}$ defined as:
\begin{equation*}
...
as:
\begin{equation*}
  S_{ij} = \frac{1}{n-1} \sum_{k=1}^{n} (\vec{s}_{ki}-\bar{\vec{s}}_i)(\vec{s}_{kj}-\bar{\vec{s}}_j),
\end{equation*}
where $\vec{s}_{ki}$ means the $i$th component of the $k$th element and
$\bar{\vec{s}}_i$ means the mean of component $i$ across $\dset$.  In this
context, components are often called \textit{features} or \textit{fields}.
The covariance matrix associated with $\dset$ is then the $m \times m$ matrix
$S = [S_{ij}]$, which is clearly symmetric.  The Mahalanobis distance, $d_{P}
\colon \dset \times \dset \to \mathbb{R}^{\geq 0}$, is then defined as:
\begin{equation*}
...
Levenshtein distance \citep{levenshtein1965distance}.  The Levenshtein
distance is an edit distance, where the allowed edits are insertion, deletion
and substitution (for definitions, see \citep{levenshtein1965distance}).  The
...
Let $\dset$ be a set.  The cardinality of the symmetric difference,
$\delta_{\symdif} \colon 2^{\dset} \times 2^{\dset} \to \mathbb{R}^{\geq 0}$
is a well-known metric on sets defined as:
\begin{equation*}
...
\begin{figure}
  ...
  \caption{$X$ and $Y$ are sets with elements labelled by \tikzuptriangle ~and
    ...
    \tikzbotriangle .  The Haussdorf distance between $X$ and $Y$ is,
...
Let $(M,d)$ be a metric space and $\mathcal{M}=2^M \setminus \emptyset$.  The
Haussdorf distance, $\delta_{H} \colon \mathcal{M} \times \mathcal{M} \to
\mathbb{R}^{\geq 0}$, is defined as:
\begin{equation*}
  \delta_{H}(X,Y) = \max\left(\max_{x \in X} \min_{y \in Y} d(x,y),
                             \max_{y \in Y} \min_{x \in X} d(x,y)\right), 
\end{equation*}
for all $X,Y \in \mathcal{M}$.  The Haussdorf distance is a metric on
$\mathcal{M}$ \citep{braun2003geometry}.  This distance is illustrated in
Figure~\ref{fig:haussdorf} by showing the Haussdorf distance between two sets
...
generally metrics.  For a metric space, $(M,d)$, and $\mathcal{M}=2^M
...
discussed further in Section~\ref{sec:hierarchies}.
...
$n > 0$ and refer to a set of $n$ elements as an \textit{$n$-set}.  We also
denote the set $\{1,\dotsc,n\}$ by $[n]$.

\subsection{The space of partitions}
\label{sec:space-partitions}

Given an $n$-set $\dset$, a $k$-partition $\clus = \{C_1,C_2,\dotsc,C_k\}$ of
$\dset$ is a set of $k \in [n]$ nonempty, pairwise-disjoint subsets of $\dset$
such that $C_1 \cup C_2 \cup \dotsb \cup C_k = \dset$.  Following common
practice, we will refer to the elements of $\clus$ as \textit{clusters}.
...
Note that in our definition, all members of the underlying set are members of
the fuzzy set to some extent since the grades of membership cannot be zero.

A \textit{$k$-fuzzy-partition} of an $n$-set $\dset$ is a set of $k \in [n]$
fuzzy sets: \[\{(C_1,\mu_1),(C_2,\mu_2),\dotsc,(C_k,\mu_k)\}\] where $C_1 \cup
\dotsb \cup C_k = \dset$, $C_i \neq \emptyset$ for all $i \in [k]$ and
$\sum_{i=1}^{k} \mu_i(x) = 1$ for all $x \in \dset$.  Note that the underlying
sets of the fuzzy sets $(C_i,\mu_i)$ are not necessarily pairwise disjoint, so
...
$\dset^* = [9]$ throughout this section:
\begin{equation}
...
similarity between $\clus_1$ and $\clus_2$.  It finds a matching using the
following heuristic: let $[n_{ij}]$ be the $k \times k'$ confusion matrix
associated with $\clus_1$ and $\clus_2$.  Compute $n_{ab} = \argmax \{n_{ij}
\colon 1 \leq i \leq k, 1 \leq j \leq k'\}$ and put $\sigma(a) \gets b$.
Repeat the process for the $(k-1) \times (k'-1)$ submatrix obtained by
deleting row $a$ and column $b$, and so on until $k$ matches have been made.
This process give us an injection for $\sigma$ and the measure is then defined
as:
\begin{equation*}
...
this type of measure in Chapter~\ref{cha:sum-squar-clust}.
...
then it is implicitly using Euclidean distance.
...
Given the potentially huge space of possible partitions of a dataset,
partitional clustering is intuitively a hard problem.  In fact, as we will see
below, it has been shown that many partitional clustering problems are
NP-complete.
...
refer to simply as \textsc{Clustering} and, again, whenever we speak of a
particular criterion we will qualify the name appropriately, so for example
\textsc{Centroid-Distance Clustering} is the problem of finding an optimal
...
\noindent \textsc{Sum-of-Cuts Clustering} (\textsc{S-Cuts}) is easily seen to
be NP-complete be considering the following classic NP-complete graph problem
...
\end{problem}This is clearly a specific case of \textsc{S-Cuts} with $k=2$ and
...
even the approximation problem is NP-complete.
...
is solved with the single-link hierarchical clustering algorithm of
...
with a criterion for homogeneity in a bicriterion, effectively making it a
hard problem again \citep{delattre1980bicriterion}.
...
{X\choose 2}$ of $X$ a {\em covering} of $X$ is $X=\bigcup_{A\in \mathcal L}A$
...
\item A {\em rooted phylogenetic $X$-tree}, or $X$-tree for short, is a rooted
  tree $T=(V,E)$ with no degree two vertices but possibly the root $\rho_T$
  whose leaf set is $X$. For $T$ an $X$-tree, we call any vertex of $T$ that
  is not a leaf of $T$ an {\em interior vertex} of $T$ and an edge that is
  incident with a leaf of $T$ a {\em pendant edge} of $T$.
  ...
  \item $D_{(T,\omega)}(x,u)\geq D_{(T,\omega)}(x,v)$, for all $x\in X$ and
    any $u,v\in V$ such that $u$ is encountered before $v$ on the path from $\rho$
    to $x$.
  \end{enumerate}
