...
for the work presented later.  We then review the current state of two broad
categories of data analysis techniques, namely partitional clustering and
...
The Haussdorf distance, $\delta_{H} \colon \mathcal{M} \times \mathcal{M} \to
...
$S_k$ of all possible injections from $[k]$ to $[k']$:
\begin{equation*}
...
To give a numerical example, let $\dset = [15]$,
\begin{equation*}
...
\begin{problem}{Clustering}
  \instance{An $n$-set $\dset$, the number of clusters desired $k \in [n]$, a
    criterion $f \colon \parts_{\dset} \to \mathbb{R}^+$, which involves $k$
    in some way, and a bound $B \in \mathbb{R}^+$.}
  \question{Does there exist a partition $\clus \in \parts_{\dset}$ such that
    $f(\clus) \leq B$?}
\end{problem}
...
This section is a place holder for work yet to be completed.  Hierarchical
clustering is a more general form of partitional clustering in which a tree is
sought containing a hierarchy of clusters.  Each layer of the tree is a
partition.  We will review a few of the general techniques such as the
