...
Contributions by the candidate include: the examples showing that
...
Section~\ref{sec:assignment-metric}, and the examples leading to
Theorem~\ref{thm:worst-case}.  The candidate was also responsible for
preparation of the published manuscript and implementation of the assignment
metric for testing.
...
\begin{proof}
  ...
  Now to show that ASC is NP-complete we will

  Now we choose again to construct a transformation from PARTITION INTO
  TRIANGLES (PT).  An instance $I$ of PT is transformed into an instance
  $f(I)$ of ASC as follows: first we construct a 3-valued metric space by
  setting $\dset$ to $V$ and defining a function $d \colon \dset \times \dset
  \to \{0,1,2\}$ by
  \begin{equation*}
    d(u,v) = \begin{cases}
      0 & \text{if $u=v$,}\\
...
\end{equation*}
for some $\delta \colon 2^{\dset} \!\!\times 2^{\dset} \to \mathbb{R}$ and
where $S_k$ is the set of all possible functions $\sigma \colon \{1, \dotsc,
k\} \to \{1, \dotsc, k\}$.
...
In Section~\ref{sec:lifting-metric-space} we discuss the use of metrics which
are aware of the underlying metric space of a clustering.  In
Sections~\ref{sec:upper-bound} and \ref{sec:worst-case-perf} we use the
symmetric difference to prove an upper bound and then a worst case result for
our two clustering criteria.
...
\subsubsection{Lifting the underlying metric space}
